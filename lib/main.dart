      ...
      body: Padding(
        ...
        child: ListView.builder( 
          ...
          itemBuilder: (context, index){
            ...
            final taskColor = getStatusColor(task['status']);
              ...
              child: Container(
                ...
                child: ListTile(
                  ...
                  subtitle: Column(
                    ...
                    children: [
                      Text(
                        'Creado por: ${task['created_by'] ?? 'Desconocido'}',
                        style: TextStyle(fontSize: 12, color: Colors.black54),
                      ),
                      Text(
                        'Última modificación por: ${task['last_modified_by'] ?? 'Desconocido'}',
                        style: TextStyle(fontSize: 12, color: Colors.black54),
                      ),
                      Text(
                      ...
                      Text(
                        'Última modificación: ${formatDate(task['updated_at'])}',
                        style: TextStyle(fontSize: 12, color: Colors.black54),
                      ),
                    ],
                  ),
...
class _TaskScreenState extends State<TaskScreen> {
  ...
  // Función para obtener las tareas ordenadas por fecha más reciente
  Future<void> getTasks() async {
    final user = Supabase.instance.client.auth.currentUser;
    if (user != null) {
      final response = await Supabase.instance.client
          ...
          .select()
          .eq('user_id', user.id)
          .order('updated_at', ascending: false) // Ordenar por fecha más reciente
          .execute();
  ...
  Color getStatusColor(String status) {
    switch (status.toLowerCase()) {
      case 'pendiente':
        return Colors.red.shade200; // Rojo claro
      case 'en progreso':
        return Colors.orange.shade200; // Naranja claro
      case 'completada':
        return Colors.green.shade200; // Verde claro
      default:
        return Colors.grey.shade300; // Gris claro (predeterminado)
    }
  }
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Expanded(
              child: ListView.separated( // Usamos ListView.separated para añadir líneas divisorias
                itemCount: tasks.length,
                separatorBuilder: (context, index) => Divider(height: 1, color: Colors.black12), // Línea divisoria
                itemBuilder: (context, index) {
                  final task = tasks[index];
                  final taskColor = getStatusColor(task['status']); // Obtener el color según el estado

                  return Container(
                    color: taskColor, // Aplicar el color al contenedor padre
                    child: ListTile(
                      title: Text(
                        task['title'],
                        style: TextStyle(color: Colors.black), // Asegurarse de que el texto sea legible
                      ),
                      subtitle: Text(
                        task['description'],
                        style: TextStyle(color: Colors.black), // Asegurarse de que el texto sea legible
                      ...
                      trailing: Container(
                        padding: EdgeInsets.symmetric(horizontal: 8, vertical: 4),
                        decoration: BoxDecoration(
                          color: taskColor, // Usar el mismo color para el estado
                          borderRadius: BorderRadius.circular(5),
                        ),
                        child: Text(
                          task['status'],
